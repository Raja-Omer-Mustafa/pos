...
class Pos extends MY_Controller
{
    ...
    public function getProductDataByCode($code = NULL, $warehouse_id = NULL)
    {
        ...
        if ($row) {
            ...
            // echo '<pre>';
            // echo print_r($row->type);
            // echo '</pre>';
            
            if ($row->type == 'standard' && (!$this->Settings->overselling && $row->quantity < 1)) {
            // if (($row->type == 'hardware' || $row->type == 'timber' || $row->type == 'sheets' || $row->type == 'panel') && (!$this->Settings->overselling && $row->quantity < 1)) {
                echo NULL; die();
            }
            if ($options) {
                $option_quantity = 0;
