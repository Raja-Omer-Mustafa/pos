...
class Purchases extends MY_Controller
{
    ...
    {
        // echo strtolower($this->router->fetch_class());
        // die();
        ...
        // $this->page_construct('purchases/index');
...
//generate pdf and force to download
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addPurchase($data, $products)) {
            ...
            if ($quote_id) {
                ...
                foreach ($items as $item) {
                    ...
                    // elseif ($row->type == 'hardware' || $row->type == 'timber' || $row->type == 'sheets' || $row->type == 'panel') {
                        if (!$row) {
                            $row = json_decode('{}');
                            $row->quantity = 0;
                        } else {
    ...
    public function add_supplier_payment() // $id = null
    {
        $this->sma->checkPermissions();
        // $this->sma->checkPermissions('payments', true);
        $this->load->helper('security');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $payment = array(
                ...
                'supplier_id' => $this->input->post('supplier_id'), // posupplier
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addPayment($payment)) {
            ...
            $this->load->view($this->theme . 'purchases/add_supplier_payment', $this->data);
        }
    }

    public function supplier_payment($warehouse_id = null)
    {
        $this->sma->checkPermissions('payments'); // action , class
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['users'] = $this->reports_model->getStaff();
        $this->data['billers'] = $this->site->getAllCompanies('biller');
        $this->data['pos_settings'] = POS ? $this->reports_model->getPOSSetting('biller') : FALSE;

        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('purchases'), 'page' => lang('purchases')), array('link' => '#', 'page' => lang('list_payment')));
        $meta = array('page_title' => lang('list_payment'), 'bc' => $bc);
        ...
        $this->page_construct('purchases/supplier_payment', $meta, $this->data);
        // echo strtolower($this->router->fetch_class());
        // die();
    ...
    {
        // $this->sma->checkPermissions('payments', true);
        $this->sma->checkPermissions(); // action-class
        $this->load->helper('security');
        if ($this->input->get('id')) {
            $id = $this->input->get('id');
        }
    ...
    function get_supplier_payments($pdf = NULL, $xls = NULL)
    {
        $this->sma->checkPermissions('payments', TRUE);
