...
class Sales extends MY_Controller
{
    ...
    public function extra_sale()
    {
        $this->sma->checkPermissions('extra_sale', true);
        $error = null;
        $esData = $this->input->get('gcdata');
        // $this->sma->send_json(array('result' => 'failed', 'message' => $esData));
        if (empty($esData[0])) {
            $error = lang("product_code") . " " . lang("is_required");
        }
        if (empty($esData[1])) {
            $error = lang("product_name") . " " . lang("is_required");
        }
        if (empty($esData[2])) {
            $error = lang("supplier") . " " . lang("is_required");
        }
        // if (empty($esData[3])) {
        //     $error = lang("date") . " " . lang("is_required");
        // }
        if (empty($esData[4])) {
            $error = lang("price") . " " . lang("is_required");
        }
        $supplier_details = (!empty($esData[2])) ? $this->site->getCompanyByID($esData[2]) : null;
        $data = array(
            'product_code' => $esData[0],
            'product_name' => $esData[1],
            'supplier_id' => $esData[2],
            'date' => date('Y-m-d H:i:s'),
            // 'date' => $esData[3],
            'price' => $esData[4],
            'supplier' => $supplier_details->company."-".$supplier_details->name,
            'created_by' => $this->session->userdata('user_id'),
        );

        if (!$error) {
            if ($this->sales_model->addExtraSale($data)) {
                $this->sma->send_json(array('result' => 'success', 'message' => lang("extra_sale_added")));
            }
        } else {
            $this->sma->send_json(array('result' => 'failed', 'message' => $error));
        }
