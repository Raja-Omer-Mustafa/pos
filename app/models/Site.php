...
class Site extends CI_Model
{
    ...
    public function item_costing($item, $pi = NULL) {
        ...
        if ($this->Settings->accounting_method != 2 && !$this->Settings->overselling) {
            ...
            if ($this->getProductByID($item['product_id'])) {
                ...
                // if (($item['product_type'] == 'hardware' || $item['product_type'] == 'timber' || $item['product_type'] == 'sheets' || $item['product_type'] == 'panel')) {
                    ...
                    foreach ($combo_items as $combo_item) {
                        ...
                        if ($pr->type == 'standard') {
                        // if ($pr->type == 'hardware' || $pr->type == 'timber' || $pr->type == 'sheets' || $pr->type == 'panel') {
    ...
    public function costing($items) {
        ...
        foreach ($items as $item) {
            ...
            // if ($pr && ($pr->type == 'hardware' || $pr->type == 'timber' || $pr->type == 'sheets' || $pr->type == 'panel')) {
                ...
                foreach ($combo_items as $combo_item) {
                    ...
                    // if ($combo_item->type == 'hardware' || $combo_item->type == 'timber' || $combo_item->type == 'sheets' || $combo_item->type == 'panel') {
    ...
    public function checkSlug($slug, $type = NULL) {
        if (!$type) {
            // echo '<pre>';
            // echo print_r($this->db->get_where('products', ['slug' => $slug], 1)->row());
            // echo '</pre>';
            // die();
