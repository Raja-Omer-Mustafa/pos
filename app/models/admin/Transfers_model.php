...
class Transfers_model extends CI_Model
{
    ...
    public function getProductNames($term, $warehouse_id, $limit = 5)
    {
        ...
        if ($this->Settings->overselling) {
            ...
            // $this->db->where("(type = 'hardware' OR type = 'timber' OR type = 'sheets' OR type = 'panel') AND (name LIKE '%" . $term . "%' OR code LIKE '%" . $term . "%' OR  concat(name, ' (', code, ')') LIKE '%" . $term . "%')");
        } else {
            $this->db->where("type = 'standard' AND warehouses_products.warehouse_id = '" . $warehouse_id . "' AND warehouses_products.quantity > 0 AND " . "(name LIKE '%" . $term . "%' OR code LIKE '%" . $term . "%' OR  concat(name, ' (', code, ')') LIKE '%" . $term . "%')");
            // $this->db->where("(type = 'hardware' OR type = 'timber' OR type = 'sheets' OR type = 'panel') AND warehouses_products.warehouse_id = '" . $warehouse_id . "' AND warehouses_products.quantity > 0 AND " . "(name LIKE '%" . $term . "%' OR code LIKE '%" . $term . "%' OR  concat(name, ' (', code, ')') LIKE '%" . $term . "%')");
        }
        $this->db->limit($limit);
