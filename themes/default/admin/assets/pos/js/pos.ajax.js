...
$('#posdiscount').focus(function () {
     ...
     * Extra Sale modal
     ...
     $(document).on('click', '#extraSale', function (e) {
        $('.error-con').hide();
        if (count == 1) {
            positems = {};
        ...
        $('.gcerror-con').hide();
        $('#esModal').appendTo("body").modal('show');
        return false;
     });
     ...
     $('#product_num').click(function(){
        $(this).parent().parent('.input-group').children('input').val(generateCardNo(8));
        return false;
    });
    ...
    $(document).on('click', '#addExtraSale', function (e) {
        // console.log("akjsgdh");
        var mid = (new Date).getTime(),
        prod_code = $('#prod_code').val(),
        prod_name = $('#prod_name').val(),
        essupplier = $('#essupplier').val(),
        // date = $('#date').val(),
        esprice = parseFloat($('#esprice').val());
        if(prod_code == '' || prod_name == '' || essupplier == '' /*|| date == ''*/ || esprice == '') {
            // console.log("yes");
        ...
        var es_data = new Array();
        es_data[0] = prod_code;
        es_data[1] = prod_name;
        es_data[2] = essupplier;
        // es_data[3] = date;
        es_data[4] = esprice;

        $.ajax({
            type: 'get',
            url: site.base_url+'sales/extra_sale',
            dataType: "json",
            data: { gcdata: es_data },
...
function loadItems() {
    ...
    if (localStorage.getItem('positems')) {
        ...
        $.each(sortedItems, function () {
            ...
            // if ((item_type == 'hardware' || item_type == 'timber' || item_type == 'sheets' || item_type == 'panel') && item.options !== false) {
                $.each(item.options, function () {
            ...
            // } else if((item_type == 'hardware' || item_type == 'timber' || item_type == 'sheets' || item_type == 'panel') && base_quantity > item_aqty) {
                $('#row_' + row_no).addClass('danger');
                ...
                if(combo_items === false) {
                    ...
                    $.each(combo_items, function(){
                        ...
                        // if(parseFloat(this.quantity) < (parseFloat(this.qty)*base_quantity) && (this.type == 'hardware' || this.type == 'timber' || this.type == 'sheets' || this.type == 'panel')) {
                            $('#row_' + row_no).addClass('danger');
                        }
                    });
                }
